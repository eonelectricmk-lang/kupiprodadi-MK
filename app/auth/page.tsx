        ...
        <Container className="py-8">
          <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1fr_0.9fr] lg:items-start">
            ...
            >
              ...
              <div className={`mt-6 rounded-2xl border p-4 text-sm ${dark ? 'border-[#1f3047] bg-black/20 text-slate-300' : 'border-slate-200 bg-white/95 text-slate-700 shadow-sm'}`}>
            ...
            <section className={`overflow-hidden rounded-[28px] border shadow-2xl lg:mt-2 ${dark ? 'border-[#1d2c43] bg-[#0b1423] shadow-black/20' : 'border-slate-200 bg-white shadow-slate-300/40'}`}>
              <div className={`border-b px-5 py-4 sm:px-8 ${dark ? 'border-[#1d2c43] bg-[#08101c]' : 'border-slate-200 bg-[#0b1321]'}`}>
                <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${dark ? 'border border-[#223653] bg-[#0b1727] text-slate-300' : 'border border-slate-200 bg-white text-slate-800'}`}>
                  <ShieldCheck className="h-4 w-4 text-emerald-400" />
                  Брз и безбеден пристап
                </div>
                <h2 className={`mt-4 text-2xl font-black tracking-tight sm:text-3xl ${dark ? 'text-white' : 'text-white'}`}>
