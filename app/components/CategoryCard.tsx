...
export default function CategoryCard({ icon: Icon, iconClassName, title, count, href }: CategoryCardProps) {
  ...
  return (
    <Link
      href={href}
      className={`group flex items-center gap-2 rounded-xl border-2 p-2 transition hover:-translate-y-0.5 ${
    ...
    >
      ...
      <div className="min-w-0">
        ...
        {count && <p className={`mt-0.5 text-[11px] ${dark ? 'text-slate-400' : 'text-gray-500'}`}>{count} огласи</p>}
      </div>
